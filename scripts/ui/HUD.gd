...
func _ready() -> void:
	...
	# Mission HUD portrait removed — it blocked gameplay. Hub shop keeps large outfit preview.
	_remove_survivor_portrait_if_present()

func _remove_survivor_portrait_if_present() -> void:
	if root == null:
		return
	for n in ["SurvivorPortrait", "SurvivorCaption"]:
		if root.has_node(n):
